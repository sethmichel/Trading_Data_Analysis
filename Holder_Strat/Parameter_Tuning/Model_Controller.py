...
market_data_dict_by_ticker = Helper_Functions.Load_Market_Data_Dictionary(bulk_df_all_values) # {date: {ticker: dataframe, ticker2: dataframe, ...}, date: ...}
...
def Load_Target_Model_And_Data():
    ...
    model, scaler, smearing_factor = Target_Model_Training.Load_Model_Data()
...
def Train_Target_Model():
    ...
    bulk_df = bulk_df_all_values[bulk_df_all_values['Original Percent Change'] > -0.3]
    ...
    if (load_model_data == False):
        ...
        roi_dictionary, trade_end_timestamps, trade_start_indexes = Helper_Functions.Create_Roi_Dictionary_For_Trades(
                bulk_df, market_data_dict_by_ticker, largest_sl_value=-0.4, holding_value=0.6) # largest_sl_value, holding_value=0.6, holding_sl_value=0

        # collect data
        all_data_samples_x, all_roi_samples_y = Target_Model_Training.Collect_Data(bulk_df, market_data_dict_by_ticker, roi_dictionary, trade_end_timestamps, trade_start_indexes)
        ...
        # save model data
        Target_Model_Training.Save_Model_Data(model, scaler, smearing_factor)

        print("="*30)
        print(f"TARGET MODEL MODEL TRAINING SUMMARY {target_model_version}\n")
        print(f"Model: {len(bulk_df)} training samples")
        print("="*30)
...
def Load_Sl_Model_And_Data():
    ...
    # load model and training data
    trades_processed, trades_dropped_neither, neither_trades_detected = Sl_Model_Training.Load_Training_data()
    model, scaler, result_df = Sl_Model_Training.Load_Model_data()

    return result_df, trades_processed, trades_dropped_neither, neither_trades_detected, model, scaler
...
def Train_Sl_Model():
    ...
    if (load_model_data == False):
        ...
        # NOTE: this function loads them if they already exist
        roi_dictionary, trade_end_timestamps, trade_start_indexes \
                = Helper_Functions.Create_Roi_Dictionary_For_Trades(bulk_df_all_values, market_data_dict_by_ticker, max(sl_list))
        
        # collect training data
        results_df, trades_processed, trades_dropped_neither, neither_trades_detected \
                = Sl_Model_Training.Collect_data(
                        sl_list, bulk_df_all_values, market_data_dict_by_ticker, roi_dictionary, trade_start_indexes, drop_neither_trades)

        # save training data
        Sl_Model_Training.Save_Training_Data(results_df, trades_processed, trades_dropped_neither, neither_trades_detected)
...
def Sl_Model_Run_Diagnostics():
    # load data/model
    results_df, trades_processed, trades_dropped_neither, \
    neither_trades_detected, model, scaler = Load_Target_Model_And_Data()
...
#Train_Target_Model(bulk_df_all_values)